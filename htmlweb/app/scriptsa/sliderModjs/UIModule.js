define(['sMods/TemplateModule', 'sMods/TooltipModulesto', 'sMods/HighLight', 'sMods/SliderModule', 'sMods/ButtonModule', 
        'sMods/DoughNutModule', 'sMods/ScorerModule', 'sMods/SliderModuleYN','sMods/Helper','tinysort'], 
    function(TemplateModule, TooltipModulesto, HighLight, SliderModule, ButtonModule, DoughNutModule, ScorerModule, SliderModuleYN,
             Helper,tinysort){
    "use strict";
    return function () {
        var elementArray = [];
        ...
        return {
            init: function (numberOfElements, numberOfButtons, uiType) {
                ...
                this.buttonLabels = ['Manager', 'Goal Keeper & Defence', 'Midfield', 'League Position & Recent Form', 'Attack','Owner'];
                this.scorerValues = [];
                ...
                if(this.windowWidth <= 1200){
                    document.getElementById('sliderLabel4').className = this.helper.addClass('sliderLabel4', 'hideVisibility');
                }
            ...
            slider: function () {
                ...
                if (this.windowWidth < 992) {
                    this.helper.updateClassNamesAdd('ui-slider-handle', ' fa fa-minus-square fa-lg fa-rotate-90 handleStyle');
                    this.helper.updateClassNamesAdd('tooltip1', ' top');
                    this.windowResizeBool = true;
                }
                else {
                    this.helper.updateClassNamesAdd('ui-slider-handle', ' fa fa-minus-square fa-2x handleStyle');
            ...
            sliderYN: function() {
                ...
                var buttonLabels = ['Manager', 'Goal Keeper & Defence', 'Midfield', 'League Position & Recent Form', 'Attack', 'Owner'];
                var templateModule = new TemplateModule();
                this.windowResizeBool = true;
                // add a label for each slider
                for (var i = 0; i < this.numberOfbuttons; i++) {
                ...
                for (var i = 0; i < this.numberOfButtons; i++) {
                    buttonArray[i] = new ButtonModule();
                    buttonArray[i].setButtonID(i);
                    templateModule.createButton(i, buttonLabels[i],this);
                    elementArray[i] = new SliderModuleYN();
                    elementArray[i].setSliderID(i);
                    templateModule.createSliderYN(i, this);
                }
                ...
                for (var i = 0; i < this.numberOfButtons; i++) {
                    document.getElementById('sliderID' + i).className = this.helper.addClass('sliderID' + i, ' overlaydiv overlayDiv1 handleStyle');
                }
                ...
                if (this.windowWidth < 992) {
                    ...
                    this.helper.updateClassNamesAdd('ui-slider-handle', ' fa fa-minus-square fa-2x fa-rotate-90 handleStyle');
                }
            ...
            setButtonValue: function (buttonID, self) {
                ...
                var removeClass = this.helper.removeClass;
                var addClass = this.helper.addClass;
                // update the total value for the slider
                for (var i = 0; i < this.numberOfElements; i++) {
                    if(modules.baseURI.charAt(modules.baseURI.length-1) == 3){
            ...
            updateButtonValue: function (buttonNumber) {
                ...
                if (buttonArray[buttonNumber].buttonUpdateID !== null) {
                    ...
                    for (var sliderNumber = 0; sliderNumber < this.numberOfElements; sliderNumber++) {
                        ...
                        if(modules.baseURI.charAt(modules.baseURI.length-1) == 3){
                            var sliderValue = elementArray[sliderNumber].totalValue / 10;
                        }
                        else {
                        ...
                        chartObject.segments[segment2].value = sliderValue;
                        chartObject.update();
                     //   this.updateTotathis.helper.l(sliderNumber);
                    }
                }
                else {
                    for (var sliderNumber = 0; sliderNumber < this.numberOfElements; sliderNumber++) {
                ...
                $('.results').children('li').each(function () {
                    ...
                    switch(count) {
                        case 1:
                            ...
                            document.getElementById('sliderTotalSmall' + thisID[0]).className = self.helper.addClass('sliderTotalSmall' + thisID[0], 'goldBadge');
                        ...
                        case 2:
                            ...
                            document.getElementById('sliderTotalSmall' + thisID[0]).className = self.helper.addClass('sliderTotalSmall' + thisID[0], 'silverBadge');
                            this.className = 'defaultChartHeader';
                            break;
                        case 3:
                            $('#canvas'+thisID[0]).css('display','none');
                            document.getElementById('sliderTotalSmall' + thisID[0]).className = self.helper.addClass('sliderTotalSmall' + thisID[0], 'bronzeBadge');
                            this.className = 'defaultChartHeader';
                            break;
                        default:
                            $('#canvas' + thisID[0]).css('display', 'none');
            ...
            setSliderYNValue: function (sliderValue, sliderID) {
                ...
                var sliderID = sliderID.charAt(sliderID.length-1);
                // update the slider module values each time the slider moves
                elementArray[sliderID].setValues(sliderValue.value, buttonArray[sliderID].buttonValue);
            ...
            setButtonValueYN: function (buttonID) {
                ...
                if (buttonArray[buttonID].buttonValue === 1) {
                    $('#moduleButton' + buttonID).children().this.helper.removeClass('overlayDiv' + 5);
                }
                else { $('#moduleButton' + buttonID).children().this.helper.removeClass('overlayDiv' + (buttonArray[buttonID].buttonValue - 1)); }
                $('#moduleButton' + buttonID).children().this.helper.addClass('overlayDiv' + buttonArray[buttonID].buttonValue);
                // this update the total value
