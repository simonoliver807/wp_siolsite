...
define(['oimo', 'three','orbitControls'], function(OIMO,THREE,orbitControls) {
...
//                            });
    ...
    var material = new THREE.MeshBasicMaterial({color:0xff0000,wireframe:true});    
    var gGeo = new THREE.PlaneGeometry(400,390,512,512);
    var g = new THREE.Mesh(gGeo,material);
    g.position.y = -0.5;
    g.rotation.x = -Math.PI/2; //-90 degrees around the xaxis
    g.name = 'g';
    ...
    var s1 = new THREE.Mesh( new THREE.SphereGeometry(5,16,16), sphereMaterial);
    s1.name = 's1';
    meshs[meshNum] = s1;
    meshNum += 1;
    s1.position.y = 10.0;
...
//    
    ...
    function oimoPopulate(obj){
        ...
        if (obj.name.charAt(0) === 'g') {
            ...
            oimoObj = {type:'box', size: [h, 40, w], pos:[posx, posy, posz], world:world, rot: [90,0,0], name:obj.name };
        }
